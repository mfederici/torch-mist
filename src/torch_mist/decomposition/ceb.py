...
class CEB(StochasticDimensionalityReduction):
    def __init__(
        ...
        *args,
        beta: float = 0.01,
        conditional_hidden_dims: Optional[List[int]] = None,
        **kwargs
    ):
        super().__init__(*args, **kwargs)
        if conditional_hidden_dims is None:
            conditional_hidden_dims = [128]
        self.conditional_hidden_dims = conditional_hidden_dims
        self.beta = beta
    ...
    def _instantiate_model(self, x_dim: int, y_dim: int):
        ...
        q_ZX_given_ZY = conditional_transformed_normal(
            ...
            context_dim=y_dim,
            hidden_dims=self.conditional_hidden_dims,
        )
