...
class VIB(StochasticDimensionalityReduction):
    ...
    def _add_default_model_params(
        ...
        model_params["q_ZX"] = NormalModule(
            loc=torch.zeros(self.n_dim), scale=torch.ones(self.n_dim)
        )
