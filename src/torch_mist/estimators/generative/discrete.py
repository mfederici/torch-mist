...
from torch_mist.estimators.generative.pq import SameBucketConditionalDistribution
...
class DiscreteMutualInformationEstimator(GenerativeMutualInformationEstimator):
    ...
    @cached
    def approx_log_p_qxqy(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    ...
    def approx_log_p_qy(self, y: torch.Tensor, x: Optional[torch.Tensor] = None) -> torch.Tensor:
        qy = self.quantize_y(y=y)
        ...
        assert log_q_qy.shape == y.shape[:-1], f'{log_q_qy.shape} != {y.shape[:-1]}'

        return log_q_qy

    def log_p_qx_given_qy(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        return self.approx_log_p_qxqy(x=x, y=y)-self.approx_log_p_qy(y=y)

    def approx_log_p_qy_given_qx(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
